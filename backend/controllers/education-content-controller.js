...
const DeleteEducationContent = async (req, res) => {
    const { _id } = req.query;
    try {
        await EducationContent.findByIdAndDelete(_id);
...
const EditEducationContent = async (req, res) => {
    ...
    try {
        await EducationContent.findByIdAndUpdate(id, {
            title,
            createdBy,
            description
        });
...
module.exports = { AddEducationContent, getAllEducationContent, DeleteEducationContent, EditEducationContent };